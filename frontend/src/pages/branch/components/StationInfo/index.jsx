...
function StationInfo() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const userData = await GetUserData(); // Await the userData promise
      setStationId(userData.user.station_id);
    };

    fetchData();
  }, []); // Ensure this useEffect only runs once
  ...
  useEffect(() => {
    if (stationId !== 0) {
      ...
      axios.get(`http://localhost:8000/api/get_station_by_id/${stationId}`).then((response) => {
        ...
        setOpeningTime(stationData.opening_time);
        setClosingTime(stationData.closing_time);
        setStationName(stationData.name);
      });
    }
  }, [stationId]); // Ensure this useEffect runs whenever stationId changes

  const submitHandler = () => {
    // Use the current values of state variables directly inside the handler
    axios
      .post('http://localhost:8000/api/update_station_hours', { opening_time: openingTime, closing_time: closingTime, station_id: stationId })
      .then((response) => {
    ...
    <div className="station-info flex column align-center">
      <div className="station-header full-w white-bg flex">
        ...
        <span
          ...
          className={!isWorkingHours ? 'tab-right active flex center' : 'tab-right flex center'}
        >
          Facilities
        </span>
      </div>
          ...
          <div>
            ...
            <div className="flex column center station-hours">
              ...
              <div className="secondary-bg flex align-center justify-between">
                <button className="submit-btn full-w full-h white-text" onClick={submitHandler}>
                  Submit
                </button>
              </div>
            </div>
                  ...
                  if (facility !== 'id' && facility !== 'station_id' && facility !== 'created_at' && facility !== 'updated_at') {
                    return (
                      <Facility
                        ...
                        clickHandler={() => {
                          axios
                            .post('http://localhost:8000/api/update_facility', { ...facilities, [facility]: facilities[facility] === 1 ? 0 : 1 })
                            .then((response) => {
                              if (response.data.message === 'updated successfully') {
                                setFacilities((prevFacilities) => ({ ...prevFacilities, [facility]: prevFacilities[facility] === 1 ? 0 : 1 }));
                              }
                            });
                        }}
                      />
